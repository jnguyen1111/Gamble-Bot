...
public class BadgeShop extends ListenerAdapter {
    ...
    ActionRow defActionRow = ActionRow.of(
            Button.primary("badgePage1", "1"),
            Button.secondary("badgePage2", "2"),
            Button.secondary("badgePage3", "3"),
            Button.danger("exitBadgeShop", "Exit ✖")
    );
    ActionRow secondActionRow = ActionRow.of(
            Button.secondary("badgePage1", "1"),
            Button.primary("badgePage2", "2"),
            Button.secondary("badgePage3", "3"),
            Button.danger("exitBadgeShop", "Exit ✖")
    );
    ...
    public MessageEmbed createBadgeShopEmbeds(LinkedHashMap<String, List<String>> badgeList, DataBase db) {
        ...
        badgeShopEmbed.setTitle(sussyCoinEmote + "CsLotto SOUVENIR SHOP™" + sussyCoinEmote);
        ...
        while(badgeIterator.hasNext()){
            if(pageNumberIterator == amountOfBadgesPerEmbed) { //when an embed is full, add it to the arraylist and make a new embed.
                ...
                currentEmbed = new EmbedBuilder()
                        .setTitle(sussyCoinEmote + "CsLotto SOUVENIR SHOP™" + sussyCoinEmote)
    ...
    public void printBadgeShopEmbed(MessageReceivedEvent event, DataBase db, LinkedHashMap<String, List<String>> badgeList){
        ...
        iter = badgeShopEmbedPages.listIterator(); // we create a iterator that will allow you travase the linked list forward or backward direction
    ...
    public void onButtonInteraction(@NotNull ButtonInteractionEvent event){
        ...
        switch(event.getComponentId()){
            case "badgePage1":
                if (badgeShopEmbedPages.size() >= 1) {
                    printBadgeShopEmbedPage(event, 0);
                    event.deferEdit().queue();
                }
                break;
            case "badgePage2":
                if (badgeShopEmbedPages.size() >= 2) {
                    printBadgeShopEmbedPage(event, 1);
                    event.deferEdit().queue();
                }
                break;
            case "badgePage3":
                if (badgeShopEmbedPages.size() >= 3) {
