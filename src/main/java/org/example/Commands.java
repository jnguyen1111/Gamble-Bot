...
public class Commands extends ListenerAdapter {
    ...
    //specific channel
    public String csLottoChannelID = "1137587749159964702";
    ...
    public boolean isChannelValid(MessageReceivedEvent event,String commandType){
        ...
        String errorMessage = "Please use this command in CsLotto channel!";

        if(!eventChannelID.equals(csLottoChannelID)) {
            event.getChannel().deleteMessageById(event.getChannel().getLatestMessageIdLong()).queue();
        ...
        if(args[0].charAt(0) == PREFIX){
            ...
            switch(args[0].substring(1)){
                ...
                case "addbadge":
                    if(!isChannelValid(event,"lounge")){break;}
                    if (args.length < 4){
                        event.getChannel().sendMessage("Invalid command request! please use the help command to properly use this command! "
                                + userID).queue();
                        break;
                    }
                ...
                case "beg":
                    ...
                    if(!checkUserRequestValid(event,args.length,2)){break;}
                    event.getChannel().deleteMessageById(event.getChannel().getLatestMessageIdLong()).queue();
                    event.getChannel().sendMessage(args[1] + " Please spare some <a:SussyCoin:1004568859648466974> \n-" + event.getMember().getNickname()).queue();
                ...
                case "snakeeyes":
                    if(!isChannelValid(event,"casino")){break;}
                    if(!checkUserRequestValid(event,args.length,2)) {break;}
                    snakeEyeObject.rollSnakeDice(server,event,args[1]);
                    break;
                default:
                    break;
            }
