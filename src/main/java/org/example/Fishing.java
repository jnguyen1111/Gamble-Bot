...
public class Fishing {
    ...
    ArrayList<Integer> rewardPointsList = new ArrayList<>(Arrays.asList(0, 100, 200, 300, 350, 400, 450, 500, 550, 600, 1000));
    ...
    public Fishing(){
        ...
        critterList.put(1000, new ArrayList<>(Collections.singletonList("<:Diamond:1002340462721515630>")));
    }
        ...
        if(validBalance(server,event)){
            ...
            if(didUserWin()){
                event.getChannel().sendMessage("Congratulations you caught a: " + getCritter() +
                        " you earned " + userReq + " after CsLotto Tax. " + user).queue();
                updateCredits(server,event, userReq, true);
            }
            else{
                event.getChannel().sendMessage("You caught a: " + getCritter() + " which is illegal under CsLotto conservation laws," +
                        " you have been fined 500 credits! <a:policeBear:1002340283364671621> " + user).queue();
                updateCredits(server,event, 500, false);
            }
        }
        //reset object
        clearGame();
