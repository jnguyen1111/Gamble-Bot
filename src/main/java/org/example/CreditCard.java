...
public class CreditCard {
    ...
    public void createCreditCardEmbed(String userNickname, String userPicture, String userID, String userCredits, ArrayList<String> userBadges,String userBanner) {
        creditCardEmbed.setTitle(userNickname);
        ...
        creditCardEmbed.setFooter("City: Waka Waka eh eh");
        creditCardEmbed.setTimestamp(Instant.now());

        StringBuilder description = new StringBuilder("ID:" + userID + "\n" + sussyCoinEmote + " CsLottoCoins: " + userCredits + "\n**Badges:**\n");

        //if users has badge then add to description string
        if (!userBadges.isEmpty()){for (String userBadge : userBadges) description.append(userBadge).append("\n");}
    ...
    public void printCreditCard(MessageReceivedEvent event, DataBase server) {
        ...
        createCreditCardEmbed(userNickname, userPicture, userID, userCredits, userBadges,userBanner);
